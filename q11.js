...
for (var a=0;a<20;a++){
	for(var b=0;b<20;b++){
		if(b+4<=20){
			var right = grid[a][b]*grid[a][b+1]*grid[a][b+2]*grid[a][b+3];
			//console.log("a="+a+" and b="+b+". grid[a][b]="+grid[a][b]+" grid[a][b+1]="+grid[a][b+1]+" grid[a][b+2]="+grid[a][b+2]+" grid[a][b+3]="+grid[a][b+3]);
			if (right>highest)highest=right;
		}
		if(a+4<=20 && b+4<=20){
			var rightd = grid[a][b]*grid[a+1][b+1]*grid[a+2][b+2]*grid[a+3][b+3];
			if (rightd>highest)highest=rightd;
		}
		if(a+4<=20){
			var down = grid[a][b]*grid[a+1][b]*grid[a+2][b]*grid[a+3][b];
			if (down>highest)highest=down;
		}
		if(a+4<=20 && b-4>0){
			var downl = grid[a][b]*grid[a+1][b-1]*grid[a+2][b-2]*grid[a+3][b-3];
			if (downl>highest)highest=downl;
		}
	}
}
console.log(highest);
